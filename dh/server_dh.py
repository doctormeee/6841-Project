...
app = Flask(__name__)
app.config['ENV'] = 'development'  # Set the environment to development mode
...
def get_public_key(user_id):
    # Return the public key for the specified user ID, or None if not found
    return jsonify({'public_key': stored_public_keys.get(user_id)})

# Route to register a user's public key
@app.route('/register', methods=['POST'])
def register():
    # Extract user ID and public key from the JSON request data
    user_id = request.json.get('user_id')
    public_key = request.json.get('public_key')
    # Store the public key in the dictionary with the user ID as the key
...
def send_message():
    ...
    # Store the message details in the dictionary using the message ID as the key
    ...
    # Print the stored messages dictionary to the console (for debugging)
...
def get_message(receiver_user_id):
    # Initialize an empty dictionary to store messages for the specific receiver
    message = {}
    # Loop through all stored messages to find messages addressed to the specified receiver
    ...
    # Return the messages if found, otherwise return a 404 error
    if message:
